const router = require("express").Router()

const Task = require('../models/Task.model')
const Project = require('../models/Project.model')

router.post('/tasks', (req, res, next) => {

  const { title, description, projectId } = req.body

  Task
    .create({ title, description, project: projectId })
    .then(newTask => {
      return Project.findByIdAndUpdate(projectId, { $push: { tasks: newTask._id } })
      // Si vas a incluir un then dentro de un then, retorna la promesa. llega al siguiente then
    })
    .then(() => res.sendStatus(201))
    .catch((err) => {
      console.log("Error while creating the task", err)
      res.status(500).json({ message: "Error while creating the task", error: err })
    })
})

module.exports = router